...
public:
	...
	void Draw()
	{
		setlinecolor(RGB(i, j, k));
		setfillcolor(RGB(k, j, i));
...
//更新子弹位置
void UpdateBulletes(std::vector<Bullet>& bullet_list, const Player& player)
{
...
int main()
{
	...
	while (running)
	{
		...
		{
			player.Move();//移动玩家
			UpdateBulletes(bullet_list, player);//更新子弹位置
			for (int i = 0; i < 6; i++)
			{
